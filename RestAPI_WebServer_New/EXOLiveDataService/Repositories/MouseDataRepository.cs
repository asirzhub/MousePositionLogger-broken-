...
{
    public class MouseDataRepository : IMouseDataRepository
    {
        private readonly ExoLiveDataContext _context;
        public MouseDataRepository(ExoLiveDataContext context)
        {
        ...
        {
            var mousedataDelete = await _context.MouseData.FindAsync(id);
            _context.MouseData.Remove(mousedataDelete);
            await _context.SaveChangesAsync();
        }
